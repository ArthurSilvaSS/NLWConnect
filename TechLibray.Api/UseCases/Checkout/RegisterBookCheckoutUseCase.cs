...
namespace TechLibray.Api.UseCases.Checkout;
...
public class RegisterBookCheckoutUseCase
{
    ...
    public void Execute(Guid bookId)
    {
        ...
        var entity = new Domain.Entidades.Checkout
        {
            UserId = user.Id,
            BookId = bookId,
    ...
    private void Validate(TechLibraryDbContext dbContext, Guid bookId)
    {
        ...
        if (book is null)
            throw new NotFundException("Livro nao encontrado");
       ...
       var amountBookNotReturned = dbContext
        ...
        if (amountBookNotReturned == book.Amount)
            throw new NotFundException("Livro nao esta disponivel para emprestimo");
    }
}
